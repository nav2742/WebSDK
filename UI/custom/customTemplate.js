(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		let chatInitialize = this.chatInitialize
		var messageHtml = '';
		if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "dropdown_template") {
		...
		} else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "login_template") {
			messageHtml = $(this.getChatTemplate("login_template")).tmpl({
				'msgData': msgData,
				'helpers': this.helpers,
				'extension': this.extension
			});
			...
			$(document).ready(function(){
		...
		}else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "calendar_template") {
			  messageHtml = $(this.getChatTemplate("calendar_template")).tmpl({
				msgData: msgData,
				helpers: this.helpers,
				extension: this.extension,
			  });
			  ...
			  today = currentMonth + "-" + currentDate + "-" + currentYear;
			  $(messageHtml).find("#demoDatePicker").datepicker({
        //   beforeShowDay: $.datepicker.noWeekends,
                ...
                onSelect: function () {
				date = $(messageHtml).find("#demoDatePicker").datepicker("getDate");
            ...
            today = currentMonth + "/" + currentDate + "/" + currentYear;
            today_format = Month_Name + "/" + currentDate + "/" + currentYear;
          }
        });
		$(messageHtml).find(".continueDate").click(function () {
          chatInitialize.sendMessage($(".chatInputBox").text(today_format), today_format, msgData);
        ...
        $(this).find(".continueButton").prop('disabled', true).css({
            'background-color': '#ddd', 
            'cursor': 'not-allowed' ,
			'pointer-events':'none'    
        });
		...
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "checkListTemplate"){
	...
	*/
	...
	customTemplate.prototype.getChatTemplate = function (tempType) {
		...
		*/
		...
		var calendar_template = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl">\
				...
				class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon"> \
					...
					{{if msgData.icon}}<div aria-live="off" class="profile-photo"> <div class="user-account avtar"></div> </div> {{/if}} \
					 <div class="messageBubble">\
            			<span class="simpleMsg">${msgData.message[0].component.payload.text}</span> \
        				</div>\
					<div class="dateContainer">\
						<div class="datepickerStyles" id="demoDatePicker"> </div>\
						<div class="continueDate">\
		...
		var login_template = '<script id="login_template" type="text/x-jquery-tmpl">\
			...
			{{if msgData.message}} \
				<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon" data-kr-msg-id="${msgData.messageId}"> \
					<div class="buttonTmplContent"> \
						...
						{{if msgData.icon}}<div class="profile-photo"> <div class="user-account avtar"></div> </div> {{/if}} \
		...
		*/
		var likeDislikeTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
			{{if msgData.message}} \
				<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon quickReplies" data-kr-msg-id="${msgData.messageId}"> \
					<div class="buttonTmplContent"> \
						{{if msgData.createdOn}}<div aria-live="off" class="extra-info">${helpers.formatDate(msgData.createdOn)}</div>{{/if}} \
						{{if msgData.icon}}<div aria-live="off" class="profile-photo"> <div class="user-account avtar"></div> </div> {{/if}} \
						{{if msgData.type === "bot_response"}} {{html helpers.convertMDtoHTML(msgData.message[0].component.payload.text, "bot")}} {{else}} {{html helpers.convertMDtoHTML(msgData.message[0].component.payload.text, "user")}} {{/if}} \
...
print(JSON.stringify(message)); */
	...
	{{if msgData.message}} \
	<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} {{if msgData.icon}}with-icon{{/if}}" data-kr-msg-id="${msgData.messageId}"> \
	...
	{{if msgData.message}} \
		<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon listView" data-kr-msg-id="${msgData.messageId}"> \
...
{{if msgData.message}} \
	<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" data-kr-msg-id="${msgData.messageId}"\
		...
		{{if msgData.createdOn}}<div aria-live="off" class="extra-info">${helpers.formatDate(msgData.createdOn)}</div>{{/if}} \
		{{if msgData.icon}}<div aria-live="off" class="profile-photo extraBottom"> <div class="user-account avtar"></div> </div> {{/if}} \
...
var advancedListTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
	...
	{{if msgData && msgData.icon}}<div class="profile-photo"> <div class="user-account avtar"></div> </div> {{/if}} \
...
print(JSON.stringify(message)); */
    ...
    {{if msgData && msgData.message && msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload}} \
        <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId}" class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} {{if msgData.icon}}with-icon{{/if}}mashreq-otp-validation" data-kr-msg-id="${msgData.messageId}"> \
...
var message= {
	...
	{{if msgData.createdOn}}<div aria-live="off" class="extra-info">${helpers.formatDate(msgData.createdOn)}</div>\
				...
				<div aria-live="off" class="profile-photo">\
					<div class="user-account avtar"></div>\
				</div> \
				{{/if}} \
...
var checkListTemplate = '<script id="chat_checklist_tmpl" type="text/x-jqury-tmpl"> \
        ...
        }else if(tempType==='login_template'){
			return login_template
		}else if(tempType==='calendar_template'){
...
//Below method is for template specific events
	...
	customTemplate.prototype.bindCheckListTemplates = function (ele, messageData) {
		...
		$(ele).off('click', '.chevron-subelement').on('click', '.chevron-subelement', function (e) {
			e.stopPropagation();
			$(e.currentTarget).toggleClass('open');
			var ind = $(e.currentTarget).attr('subElementIndex');
